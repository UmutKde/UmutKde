### Hi there 👋

I am a Computer Engineering student focused on **Backend Development** using the **.NET ecosystem**. I am passionate about building clean, scalable architectures and solving algorithmic problems.

---

#### 🛠 Technical Stack

* **Languages:** C#, SQL, JavaScript
* **Frameworks:** .NET Core, ASP.NET Web API, Entity Framework Core
* **Architecture & Patterns:** N-Tier Architecture, Generic Repository Pattern, DTOs, SOLID Principles
* **Tools:** Git, AutoMapper, MS SQL Server, Postman
* **Environment:** MacOS (Development) & Windows

#### 🔭 Currently Working On

* Building a **Library Management System** focused on backend architecture and CRUD operations.
* Solving algorithm problems on **HackerRank** to enhance logic building skills.
* Improving technical English proficiency for global opportunities.

---

![Github Stats](https://github-readme-stats.vercel.app/api?username=UmutKde&show_icons=true&theme=graywhite&hide_border=true&count_private=true)

[https://www.linkedin.com/in/umut-k%C3%B6de/] • [umutkode@gmail.com]
